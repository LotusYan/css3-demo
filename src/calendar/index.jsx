    ...
    render: function () {
        return (
            <div className={style.headerborder}>
                <p style={{ "lineHeight": 30 }}>{this.state.month}月</p>
                <p>{this.state.year}年</p>
                <p className={style['triangle-left']} onClick={this.handleLeftClick}> </p>
                <p className={style['triangle-right']} onClick={this.handleRightClick}> </p>
            </div>
        )
    }
});
var CalendarBody = React.createClass({
    ...
    render: function () {
        ...
        var node2 = arry2.map(function (item,i) {
            return (curday == item) ? <li key={i} style={{ "backgroundColor": "#eee" }}>{item}</li> : <li key={i}>{item}</li>
        })
        console.log(node1)
        console.log(node2)
        return (
            <div>
                ...
                <div className={style.CalendarDay} ref="CalendarDay"><ul>{node1}{node2}</ul></div>
            </div>
        )
    }
});
